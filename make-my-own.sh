...
(
  set -x
  find "$base"/src/main -type f -exec sed -i s/examplemod/"$modid"/g\;s/com.examplemod/"$package_name"/g {} +
  sed -i s/com.examplemod/"$package_name"/g\;s/examplemod/"$modid"/g "$base"/gradle.properties
  ...
  mv "$base"/src/main/java/com/examplemod/* "$base"/src/main/java/"$package_dir"
  mv "$base"/src/main/kotlin/com/examplemod/* "$base"/src/main/kotlin/"$package_dir"
